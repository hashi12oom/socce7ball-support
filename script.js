...
function showUser(u){
 currentUser=u;$("app").classList.remove("hidden");$("loginBtn").textContent=u.username||"Discord User";$("loginBtn").classList.add("logged-in");$("username").textContent=u.username||"Discord User";$("userid").textContent=u.id||"";
 ...
 $("staffSwitch").checked=false;updateStaffMode(false);
}
function updateStaffMode(on){if(!currentUser?.isStaff)on=false;document.body.classList.toggle("staff-mode",on);$("staffTab").classList.toggle("hidden",!on);if(!on&&$("staffView").classList.contains("hidden")===false)setView("tickets");}
...
async function loadMessages(id,stick=true){if(refreshingMessages)return;refreshingMessages=true;try{const nearBottom=$("messages").scrollHeight-$("messages").scrollTop-$("messages").clientHeight<100;const ms=await api("/api/tickets/"+id+"/messages");renderMessages(ms,stick||nearBottom);}finally{refreshingMessages=false;}}
...
async function loadBlacklist(){try{const xs=await api("/api/blacklist");$("blacklistItems").innerHTML=xs.length?xs.map(x=>{const exp=x.expires_at?new Date(x.expires_at).toLocaleString():"Permanent";return '<div class="blackRow"><div><b>'+escapeHtml(x.discord_username||x.discord_user_id)+'</b><small>'+escapeHtml(x.discord_user_id)+' · '+escapeHtml(x.reason||"No reason")+' · '+escapeHtml(exp)+'</small></div><button class="tab" data-unblack="'+escapeHtml(x.discord_user_id)+'">Remove</button></div>'}).join(""):"No blacklisted users.";$("blacklistItems").querySelectorAll("[data-unblack]").forEach(b=>b.onclick=async()=>{try{await api("/api/blacklist/"+b.dataset.unblack,{method:"DELETE"});await loadBlacklist()}catch(e){alert(e.message)}})}catch(e){$("blacklistItems").textContent=e.message}}
$("loginBtn").onclick=()=>{if(!currentUser)location.href=API_BASE+"/auth/discord"};$("openBtn").onclick=async()=>{try{if(!currentUser)showUser(await api("/api/me"));$("home").classList.add("hidden");setView("new");$("app").scrollIntoView({behavior:"smooth"})}catch{location.href=API_BASE+"/auth/discord"}};$("ticketsTab").onclick=()=>setView("tickets");$("newTicketTab").onclick=()=>setView("new");$("staffTab").onclick=()=>{if(currentUser?.isStaff&&$("staffSwitch").checked)setView("staff")};$("staffSwitch").onchange=()=>updateStaffMode($("staffSwitch").checked);$("backStaffBtn").onclick=()=>{$("blacklistView").classList.add("hidden");$("staffTickets").classList.remove("hidden")};$("backBtn").onclick=()=>setView(currentUser?.isStaff?"staff":"tickets");$("logoutBtn").onclick=()=>{localStorage.removeItem("socce7ball_access_token");accessToken="";location.href=API_BASE+"/auth/logout"};
...
$("attachBtn").onclick=()=>$("fileInput").click();$("fileInput").onchange=()=>{$("fileName").textContent=$("fileInput").files[0]?.name||""};["staffSearch","staffCategory","staffStatus"].forEach(id=>$(id).oninput=renderStaff);$("blacklistTab").onclick=()=>{if(!currentUser?.isStaff)return;$("staffTickets").classList.add("hidden");$("blacklistView").classList.remove("hidden");loadBlacklist()};$("addBlacklistBtn").onclick=async()=>{const userId=$("blackUserId").value.trim(),durationHours=Number($("blackDuration").value),reason=$("blackReason").value.trim();if(!userId){alert("Discord User ID required");return}try{await api("/api/blacklist",{method:"POST",body:JSON.stringify({userId,durationHours,reason})});$("blackUserId").value="";$("blackReason").value="";await loadBlacklist()}catch(e){alert(e.message)}};$("deleteAllBtn").onclick=async()=>{if(!confirm("Delete ALL tickets permanently?"))return;try{await api("/api/tickets/delete-all",{method:"POST",body:"{}"});await loadStaffTickets()}catch(e){alert(e.message)}};
